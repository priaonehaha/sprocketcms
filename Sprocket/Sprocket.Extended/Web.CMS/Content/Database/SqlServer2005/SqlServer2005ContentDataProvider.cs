...
{
	class SqlServer2005ContentDataProvider : SqlServer2005DataProvider, IContentDataProvider
	{
		protected override string SchemaResourceNamespace { get { return "Sprocket.Web.CMS.Content.Database.SqlServer2005.schema.sql"; } }
		protected override string ProceduresResourceNamespace { get { return "Sprocket.Web.CMS.Content.Database.SqlServer2005.procedures.sql"; } }
		...
		public Result Store(Page page)
		{
			try
			{
				...
				{
					SqlConnection conn = (SqlConnection)DatabaseManager.DatabaseEngine.GetConnection();
					SqlCommand cmd = new SqlCommand("StorePage", conn);
					cmd.CommandType = CommandType.StoredProcedure;
					if (page.PageID == 0)
		...
		public Page SelectPage(long pageID)
		{
			using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress))
			{
				...
				{
					conn.Open();
					SqlCommand cmd = new SqlCommand("SelectPageByPageID", conn);
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Parameters.Add(new SqlParameter("@PageID", pageID));
					cmd.Parameters.Add(new SqlParameter("@ExcludeDraft", false));
		...
		public Page SelectPageBySprocketPath(string sprocketPath)
		{
			using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress))
			{
				...
				{
					conn.Open();
					SqlCommand cmd = new SqlCommand("SelectPageByRequestPath", conn);
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Parameters.Add(new SqlParameter("@RequestPath", sprocketPath));
					cmd.Parameters.Add(new SqlParameter("@ExcludeDraft", true));
					SqlDataReader reader = cmd.ExecuteReader(CommandBehavior.CloseConnection);
					Page entity;
					if (!reader.Read())
						entity = null;
		...
		public Page SelectPageByPageCode(string pageCode)
		{
			using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress))
			{
				...
				{
					conn.Open();
					SqlCommand cmd = new SqlCommand("SelectPage", conn);
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Parameters.Add(new SqlParameter("@PageCode", pageCode));
		...
		public Result DeleteDraftRevisions(long pageID)
		{
			try
			{
				using (TransactionScope scope = new TransactionScope())
				{
					SqlConnection connection = (SqlConnection)DatabaseManager.DatabaseEngine.GetConnection();
					SqlCommand cmd = connection.CreateCommand();
					cmd.CommandText = "DeleteDraftRevisions";
					cmd.CommandType = CommandType.StoredProcedure;
		...
		public Dictionary<string, List<EditFieldInfo>> ListPageEditFieldsByFieldType(long pageRevisionID)
		{
			using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress))
			{
				...
				{
					conn.Open();
					SqlCommand cmd = new SqlCommand("ListEditFieldsForPageRevision", conn);
					cmd.CommandType = CommandType.StoredProcedure;
...
#warning change to stored procedure, not inline sql
		...
		public Result StoreEditFieldInfo(long pageRevisionID, EditFieldInfo info)
		{
			try
			{
				using (TransactionScope scope = new TransactionScope())
				{
					...
					SqlConnection connection = (SqlConnection)DatabaseManager.DatabaseEngine.GetConnection();
					SqlCommand cmd = connection.CreateCommand();
					cmd.CommandText = "StoreEditFieldInfo";
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Connection = connection;
					cmd.Parameters.Add(NewParameter("@PageRevisionID", pageRevisionID, DbType.Int64));
		...
		public Dictionary<string, List<string>> ListPageCategories(long pageRevisionID)
		{
			using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress))
			{
				...
				{
					conn.Open();
					SqlCommand cmd = new SqlCommand("ListCategoriesForPageRevision", conn);
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Parameters.Add(NewParameter("@PageRevisionID", pageRevisionID, DbType.Int64));
		...
		public Result StorePageCategories(long pageRevisionID, Dictionary<string, List<string>> categories)
		{
			try
			{
				using (TransactionScope scope = new TransactionScope())
				{
					...
					SqlCommand cmd = connection.CreateCommand();
					cmd.Connection = connection;
					cmd.CommandText = "StorePageCategory";
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Parameters.Add(new SqlParameter("@PageRevisionID", pageRevisionID));
					SqlParameter prmCatSetName = new SqlParameter("@CategorySetName", DbType.String);
					SqlParameter prmCatName = new SqlParameter("@CategoryName", DbType.String);
		...
		public Result StoreEditField_TextBox(long dataID, string text)
		{
			try
			{
				using (TransactionScope scope = new TransactionScope())
				{
					SqlConnection connection = (SqlConnection)DatabaseManager.DatabaseEngine.GetConnection();
					SqlCommand cmd = connection.CreateCommand();
					cmd.CommandText = "StoreEditField_TextBox";
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Connection = connection;
...
#warning change inline sql to stored procedure
		...
		public Result StoreEditField_Image(long dataID, long sprocketFileID)
		{
			try
			{
				using (TransactionScope scope = new TransactionScope())
				{
					SqlConnection connection = (SqlConnection)DatabaseManager.DatabaseEngine.GetConnection();
					SqlCommand cmd = connection.CreateCommand();
					cmd.CommandText = "StoreEditField_Image";
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Connection = connection;
					cmd.Parameters.Add(NewParameter("@EditFieldID", dataID, DbType.Int64));
		...
		public void LoadDataList_Image(List<EditFieldInfo> fields)
		{
			...
			using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress))
			{
				...
				{
					conn.Open();
					SqlCommand cmd = new SqlCommand("SELECT * FROM EditField_Image WHERE EditFieldID IN (" + ids + ")", conn);
					using (SqlDataReader reader = cmd.ExecuteReader(CommandBehavior.CloseConnection))
					{
						while (reader.Read())
						{
