...
namespace Sprocket.Web.CMS.Script.Parser
{
	...
	internal class InstructionList : IInstruction
	{
		...
		private bool acceptELSEInPlaceOfEND = false, isLoopBlock = false;
		...
		public void Build(List<Token> tokens, ref int nextIndex)
		{
			...
			while (nextIndex < tokens.Count)
			{
				...
				if ((!isLoopBlock && Token.IsEnd(token)) ||
					(isLoopBlock && Token.IsLoop(token)) ||
					(acceptELSEInPlaceOfEND && Token.IsElse(token)))
				{
					nextIndex++;
					return;
				}
	...
	internal class ShowInstruction : IInstruction
	{
		...
		private string RenderList(IList list, ExecutionState state)
		{
			...
			foreach (object o in list)
			{
				...
				{
					val = ((IExpression)o).Evaluate(state);
	...
	public class VariableExpression : IExpression
	{
		...
		/// <summary>
		/// Indicates the expression that will determine which index to return in the list
		/// </summary>
		...
		public object Evaluate(ExecutionState state)
		{
			...
			IVariableObject ix = o as IVariableObject;
			...
			{
				IList list = (IList)o;
				// if it's a collection but we haven't specified which index we're referring to
				if (indexExpr == null)
				{
					switch (propertyName)
					{
				...
				else
				{
					...
					object value = list[n];
					IVariableObject item = value as IVariableObject;
					if (item == null)
						return value;
					return item.EvaluateVariableProperty(propertyName, propertyToken, state);
				}
			}
			else
			{
				...
				return ix.EvaluateVariableProperty(propertyName, propertyToken, state);
			}
		}
	...
	public class ListEachInstruction : IInstruction
	{
		...
		public void Execute(ExecutionState state)
		{
			...
			{
				list = expr.Evaluate(state) as IList;
				if (list == null)
